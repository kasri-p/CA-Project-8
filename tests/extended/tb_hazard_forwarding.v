...
module tb;
    ...
    localparam integer PROGRAM_LEN = 21;
    ...
    initial begin
        ...
        // The load-use test reads effective address zero.
        data_mem[0] = 32'd37;

        // ------------------------------------------------------------
        // Register initialization: real instructions, not software NOPs.
        // ------------------------------------------------------------
        ...
        instructions[5] = enc_i(6'b001000, 5'd0, 5'd22, 16'd16);
        // addi $s6, $zero, 16

        // ------------------------------------------------------------
        // Forwarding test 1: EX/MEM -> ALU input A (ForwardA = 10).
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // Forwarding test 2: EX/MEM -> ALU input B (ForwardB = 10).
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // Forwarding test 3: MEM/WB -> ALU input A (ForwardA = 01).
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // Forwarding test 4: MEM/WB -> ALU input B (ForwardB = 01).
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // Forwarding priority: EX/MEM must beat MEM/WB.
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // Load-use test:
        // $s6 is non-zero, but 16 + (-16) gives effective address zero.
        // There is deliberately no NOP between lw and add.
        // ------------------------------------------------------------
        instructions[19] = enc_i(6'b100011, 5'd22, 5'd2, 16'hfff0);
        // lw  $v0, -16($s6)       ; $v0 = data_mem[0] = 37

        instructions[20] = enc_r(5'd2, 5'd12, 5'd3, 6'b100000);
        // add $v1, $v0, $t4       ; after one stall+forwarding: $v1 = 41

        // Load D memory and I memory like the supplied testbench.
        #8 rst = 1'b0;
        Jen = 1'b1;
        ...
        while ((commit_no < PROGRAM_LEN) && (cycle_no < MAX_CYCLES)) begin
            ...
            if (InstDone === 1'b1) begin
                ...
                if (last_commit_cycle >= 0) begin
                    ...
                    if (commit_no == 19) begin
                        // The testbench observes architectural register
                        // updates one commit behind the raw WB payload, so
                        // the load-use bubble appears before commit 19.
                        if (actual_gap != 2) begin
                            fail_flag = 1;
                            $display(
                                "ERROR: lw->add gap=%0d expected=2 (one stall)",
                                actual_gap
                            );
                        end
                        else begin
                ...
                case (commit_no)
                    ...
                    5: check_reg(5'd22, 32'd16);
                    6: check_reg(5'd8, 32'd13);

                    7: begin
                        check_reg(5'd11, 32'd9);
                        $display("PASS: EX/MEM forwarding to ALU input A");
                    end
                    ...
                    9: begin
                        ...
                        $display("PASS: EX/MEM forwarding to ALU input B");
                    end
                    ...
                    12: begin
                        ...
                        $display("PASS: MEM/WB forwarding to ALU input A");
                    end
                    ...
                    15: begin
                        ...
                        $display("PASS: MEM/WB forwarding to ALU input B");
                    end
                    ...
                    18: begin
                        ...
                        $display("PASS: EX/MEM priority over MEM/WB");
                    end

                    19: begin
                        check_reg(5'd2, 32'd37);
                        $display("CHECK: lw loaded 37 into $v0");
                    end

                    20: begin
                        check_reg(5'd3, 32'd41);
                        $display("CHECK: load-use result $v1 = 41");
                    end
        ...
        if (fail_flag) begin
            $display("FAILED_FORWARDING_AND_LOAD_USE");
            $finish(1);
        end
        else begin
            $display("ACCEPTED_FORWARDING_AND_LOAD_USE");
            $finish(0);
        end
    end
endmodule
